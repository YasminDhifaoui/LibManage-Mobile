...
public class Suppression extends AppCompatActivity {
	...
	protected void supprimerLivre() {
		if(spLivre.getSelectedItemPosition()>=0) {
			...
			db.collection("livres").document(livre.getId()) .delete()
					.addOnSuccessListener(new OnSuccessListener<Void>() {
						@Override public void onSuccess(Void aVoid) {
							// Le livre a été supprimé avec succès remplir();
							Toast.makeText(getApplicationContext(), "Livre supprimé avec succès", Toast.LENGTH_SHORT).show(); } }) .addOnFailureListener(new OnFailureListener() {
