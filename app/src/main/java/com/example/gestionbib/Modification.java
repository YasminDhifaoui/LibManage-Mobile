...
public class Modification extends AppCompatActivity {
    ...
    private void remplir() {
        db.collection("livres") .get()
                .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    @Override public void onComplete(@androidx.annotation.NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            for (QueryDocumentSnapshot document : task.getResult()) {
                                String id = document.getId(); String titre = document.getString("titre");
    ...
    protected void modifierLivre() {
        ...
        db.collection("livres").document(livre.getId()).update(map)
                .addOnSuccessListener(new OnSuccessListener<Void>() {
                    @Override
                    public void onSuccess(Void aVoid) {
                        // Le livre a été modifié avec succès remplir();
